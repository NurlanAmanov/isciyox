...
const yenimarka = new Set();
const yenimode = new Set();

for (let j = 0; j < data.length; j++) {
    yenimarka.add(data[j].brand);
    yenimode.add(data[j].model);
}

const markam = Array.from(yenimarka);
const modelim = Array.from(yenimode);
...
function addmarkalar() {
    ...
    markam.forEach((markaName) => {
        marka.innerHTML += `<option value="${markaName}">${markaName}</option>`;
    });

    modelim.forEach((modelName) => {
        model.innerHTML += `<option value="${modelName}">${modelName}</option>`;
    });

    marka.addEventListener('change', function () {
        const selectedMarka = marka.value;
...
function showcar(selectedMarka) {
  ...
  filteredData.map(item => {
      let vergimler =   item.year > 2007 ?  (item.price * 0.18)  : 'Vergisiz : '+ item.price;
      ...
      showcarlar.innerHTML = `
          <div class="card overflow-hidden min-w-2/5 max-w-52 text-start h-40 min-h-80 rounded-xl bg-white pb-3.5">
              ...
              <h1 class="text-lg font-bold m-2">  ${typeof vergimler === 'number' ? vergimler.toFixed(2) : vergimler} ${item.currency}</h1>
...
for (let i = 0; i < data.length; i++) {
  let vergimler = data[i].year > 2007 ? (data[i].price * 0.18) : 'Vergisiz ' + data[i].price;
  
  container.innerHTML += `  
      <div class="card overflow-hidden   max-w-52  text-start h-40 min-h-80 rounded-xl bg-white pb-3.5">
          <img class="w-full object-cover h-36" src="${data[i].images[0]}" alt="Masinlar"/>
          <h4 class="text-sm font-bold m-2">  ${typeof vergimler === 'number' ? vergimler.toFixed(2) : vergimler} ${data[i].currency}</h4>
...
function yoxla(){
alert('')
let yoxla=b.value== data.odometer > 50000 ? true : false
let
